...
from productions.decorators import first_isomorphism, isomorphism_P5
...
class P5():
    ...
    def apply(G: nx.Graph, isomorphism: Dict = None):
        ...
        for node in nodes_in_G:
            if G.nodes[node]['label'] == 'E' and ((node, I_node_id) in set(G.edges) or (I_node_id, node) in set(G.edges)):
