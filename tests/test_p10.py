...
def test_p10_should_be_applied_full_proper_graph_prim(Gprim):
    assert P10.apply(Gprim) is True
...
def test_p10_should_not_apply_not_in_middle_graph(G):
    G.remove_node(8)
    G.remove_node(9)
    G.add_node(8, label='E', pos=(1 / 2 + 0.1, 4), layer=0)
    G.add_node(9, label='E', pos=(1 / 2 + 0.1, 4), layer=0)
    G.add_edges_from([(8, 6), (8, 10), (8, 4), (8, 12)])
    G.add_edges_from([(9, 7), (9, 11), (9, 13)])

    assert P10.apply(G) is False


def test_p10_should_not_apply_not_in_middle_graph_prim(Gprim):
    Gprim.remove_node(8)
    Gprim.remove_node(9)
    ...
    Gprim.add_edges_from([(9, 7), (9, 11), (9, 13)])

    assert P10.apply(Gprim) is False


def test_p10_should_apply_with_added_nodes(G):
...
def test_p10_should_apply_with_added_nodes_prim(Gprim):
...
def test_p10_should_not_apply_with_added_nodes(G):
    G.add_node(14, label='E', pos=(0, 7), layer=0)
    G.remove_edge(1, 2)
    G.add_edge(1, 14)
    G.add_edge(14, 2)
...
def test_p10_should_not_apply_node_removed(G):
    for i in range(1, 12):
        if i == 5:
            continue
        _G = copy.deepcopy(G)
        ...
        assert P10.apply(_G) is False


def test_p10_should_not_apply_edge_removed(G):
    for i in range(1, 14):
        if i == 5:
            continue
        _G = copy.deepcopy(G)
...
def test_p10_should_not_apply_node_label_changed(G):
    for i in range(1, 12):
        if i == 5:
            continue
        _G = copy.deepcopy(G)
